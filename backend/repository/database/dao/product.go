 ...
 * @LastEditTime: 2024-04-16 10:09:09
...
// 获取热点商品
func (dao *ProductDao) SearchForRedis(pId []string, order string) (products []*model2.Product, err error) {
	err = dao.DB.Model(&model.Product{}).Where("id id (?)", pId).Order(order).Find(&products).Error
	return
}
